...
card_style1 = """padding: 20px; border-radius: 5px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); background-color: #f7f7f7;"""
...
def button_action(button_value):
    # st.write(f"Button {button_id} pressed!")
    try:
        # top_products= pd.read_csv(f"data/top_{button_value}.csv")
        amazon_data= pd.read_csv(f"data/{button_value}.csv")
        top_products = amazon_data.nlargest(5, 'rating').reset_index(drop=True)
    except:
        # result = search(input_value)
        result = search(button_value)
        # top_products= pd.read_csv(f"data/top_products_by_ratings.csv")
        # amazon_data= pd.read_csv(f"data/amazon_data.csv")
        # top_products= pd.read_csv(f"data/top_{button_value}.csv")
        amazon_data= pd.read_csv(f"data/{button_value}.csv")
        top_products = amazon_data.nlargest(5, 'rating').reset_index(drop=True)
    return top_products, amazon_data
...
st.write("<div style='margin-top: 50px;'><h1 style='color: #5A5A5A; text-align: center'>Amazon Product Reviews Dashboard</h1></div>", unsafe_allow_html=True)


#################################### Search layout ########################################
# Add css to make text bigger
# tabs_font_css = """ <style> div[class*="stTextInput"] label p { font-size: 20px; } </style> """
# st.write(tabs_font_css, unsafe_allow_html=True)

# cols= st.columns([4, 1])
# input_value = cols[0].text_input( "Search any product!", "")
...
#     amazon_data, top_products= button_action(input_value)
#     placeholder.empty()

################################# Create a layout with 3 columns ##############################################
st.markdown(f''' <div style= "{card_style1} background-color: white; text-align: center; margin-bottom: 10px; padding-bottom: 5px;  border: 1.5px solid orange;" >
                <p style= "font-size : 25px; "> Top Categories </p> </div> ''', unsafe_allow_html=True)
...
for i in range(0, len(button_labels), 6):
    with col1:
        if col1.button(button_labels[i], key=f"button_{i}", use_container_width=True):
            top_products, amazon_data= button_action(button_labels[i])

    with col2:
        if col2.button(button_labels[i + 1], key=f"button_{i + 1}", use_container_width=True):
            top_products, amazon_data= button_action(button_labels[i + 1])

    with col3:
        if col3.button(button_labels[i + 2], key=f"button_{i + 2}", use_container_width=True):
            top_products, amazon_data= button_action(button_labels[i + 2])
            
    with col4:
        if col4.button(button_labels[i + 3], key=f"button_{i + 3}", use_container_width=True):
            top_products, amazon_data= button_action(button_labels[i + 3])
            
    with col5:
        if col5.button(button_labels[i + 4], key=f"button_{i + 4}", use_container_width=True):
            top_products, amazon_data= button_action(button_labels[i + 4])
            
    with col6:
        if col6.button(button_labels[i + 5], key=f"button_{i + 5}", use_container_width=True):
            top_products, amazon_data= button_action(button_labels[i + 5])
...
if len(amazon_data) > 0:
    ...
    if len(best_sellers) > 0:
        ...
        title= best_sellers["title"][0].split(",")[0].split("|")[0].split("-")[0]
    ...
    st.markdown(f"<div style= '{card_style1} text-align: center;'> Please Select one of the Categories </div", unsafe_allow_html=True)